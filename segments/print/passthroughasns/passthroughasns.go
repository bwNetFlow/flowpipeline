...
func (segment PassthroughAsns) New(config map[string]string) segments.Segment {
	newsegment := &PassthroughAsns{
		Window:         60,
		ReportInterval: 10,
	...
	if config["window"] != "" {
		if parsedWindow, err := strconv.ParseInt(config["window"], 10, 64); err == nil {
			...
			if newsegment.Window <= 0 {
				log.Println("[error] PassthroughAsns: Window has to be >0.")
				return nil
			}
		} else {
			log.Println("[error] PassthroughAsns: Could not parse 'window' parameter, using default 60.")
		}
	} else {
		log.Println("[info] PassthroughAsns: 'window' set to default 60.")
	}
	...
	if config["reportinterval"] != "" {
		if parsedReportInterval, err := strconv.ParseInt(config["reportinterval"], 10, 64); err == nil {
			...
			if newsegment.ReportInterval <= 0 {
				log.Println("[error] PassthroughAsns: Reportinterval has to be >0.")
				return nil
			}
		} else {
			log.Println("[error] PassthroughAsns: Could not parse 'reportinterval' parameter, using default 60.")
		}
	} else {
		log.Println("[info] PassthroughAsns: 'reportinterval' set to default 60.")
	}
	...
	if config["filename"] != "" {
		...
		log.Println("[info] PassthroughAsns: Parameter 'filename' empty, output goes to StdOut by default.")
	}
	...
	if config["thresholdbps"] != "" {
		if parsedThresholdBps, err := strconv.ParseUint(config["thresholdbps"], 10, 32); err == nil {
			...
			log.Println("[error] PassthroughAsns: Could not parse 'thresholdbps' parameter, using default 0.")
		}
	} else {
		log.Println("[info] PassthroughAsns: 'thresholdbps' set to default '0'.")
	}
	...
	if config["thresholdpps"] != "" {
		if parsedThresholdPps, err := strconv.ParseUint(config["thresholdpps"], 10, 32); err == nil {
			...
			log.Println("[error] PassthroughAsns: Could not parse 'thresholdpps' parameter, using default 0.")
		}
	} else {
		log.Println("[info] PassthroughAsns: 'thresholdpps' set to default '0'.")
	}
	...
	if config["topn"] != "" {
		if parsedTopN, err := strconv.ParseUint(config["topn"], 10, 64); err == nil {
			...
			if newsegment.TopN <= 0 {
				log.Println("[error] PassthroughAsns: TopN has to be >0.")
				return nil
			}
		} else {
			log.Println("[error] PassthroughAsns: Could not parse 'topn' parameter, using default 10.")
		}
	} else {
		log.Println("[info] PassthroughAsns: 'topn' set to default 10.")
	}

	return newsegment
}

func (segment *PassthroughAsns) Run(wg *sync.WaitGroup) {
	defer func() {
		segment.writer.Flush()
		close(segment.Out)
		wg.Done()
	}()
	database := map[uint32]*Record{}
	...
	for {
		...
		case <-ticker.C:
			...
			sort.Slice(databaseEntries, func(i, j int) bool {
				...
				if iBytes == 0 {
					delete(database, databaseEntries[i].Asn)
				}
				jBytes := databaseEntries[j].Bytes.Reduce(rolling.Sum)
				if jBytes == 0 {
					delete(database, databaseEntries[j].Asn)
				}
			...
			for _, record := range databaseEntries {
				...
				fmt.Fprintf(segment.writer, "%s%d: %s, %s\n",
					segment.LogPrefix,
					record.Asn,
					humanize.SI(bps, "bps"),
					humanize.SI(pps, "pps"),
		...
		case msg, ok := <-segment.In:
			...
			for _, asn := range msg.ASPath {
				if asn == msg.NextHopAS {
					continue
				}
				record := database[asn]
				if record == nil {
...
func init() {
	...
	segments.RegisterSegment("passthroughasns", segment)
}
